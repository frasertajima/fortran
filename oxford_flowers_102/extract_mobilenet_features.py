...
def extract_features(data_loader, split_name, feature_extractor, global_pool, device):
...
def main():
    ...
    train_features, train_labels = extract_features(train_loader, 'train', feature_extractor, global_pool, device)
    val_features, val_labels = extract_features(val_loader, 'validation', feature_extractor, global_pool, device)
    test_features, test_labels = extract_features(test_loader, 'test', feature_extractor, global_pool, device)
    ...
    print('\nSaving binary files (TRANSPOSED for Fortran column-major)...')

    # Training data - write transpose
    train_features_path = Path(OUTPUT_DIR) / 'features_train.bin'
    train_labels_path = Path(OUTPUT_DIR) / 'labels_train.bin'
    train_features.T.tofile(train_features_path)  # Write (1280, 1020)
    train_labels.tofile(train_labels_path)
    print(f'  ✓ {train_features_path} [written as {train_features.T.shape}]')
    print(f'  ✓ {train_labels_path}')

    # Validation data - write transpose
    val_features_path = Path(OUTPUT_DIR) / 'features_val.bin'
    val_labels_path = Path(OUTPUT_DIR) / 'labels_val.bin'
    val_features.T.tofile(val_features_path)  # Write (1280, 1020)
    val_labels.tofile(val_labels_path)
    print(f'  ✓ {val_features_path} [written as {val_features.T.shape}]')
    print(f'  ✓ {val_labels_path}')

    # Test data - write transpose
    test_features_path = Path(OUTPUT_DIR) / 'features_test.bin'
    test_labels_path = Path(OUTPUT_DIR) / 'labels_test.bin'
    test_features.T.tofile(test_features_path)  # Write (1280, 6149)
    test_labels.tofile(test_labels_path)
    print(f'  ✓ {test_features_path} [written as {test_features.T.shape}]')
    print(f'  ✓ {test_labels_path}')

    # Verification
    print('\nVerification - First sample, first 5 features:')
    print(f'  Train: {train_features[0, :5]}')
    print(f'  Val:   {val_features[0, :5]}')
    print(f'  Test:  {test_features[0, :5]}')

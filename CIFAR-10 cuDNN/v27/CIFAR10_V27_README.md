...
```
cifar10_data/
├── images_train.bin  (300 MB)
├── labels_train.bin  (200 KB)
├── images_test.bin   (60 MB)
└── labels_test.bin   (40 KB)
```
...
Conv1: 32 filters, 3x3, ReLU + MaxPool(2x2)  → 16x16x32
Conv2: 64 filters, 3x3, ReLU + MaxPool(2x2)  → 8x8x64
Conv3: 128 filters, 3x3, ReLU + MaxPool(2x2) → 4x4x128

Flatten → 2048 features
Dense1: 2048 → 512, ReLU, Dropout(0.5)
Dense2: 512 → 10 (softmax)
...
```
CIFAR-10/
├── prepare_cifar10_v27.py       # Stage 1: Python preprocessing
├── cifar10_cudnn_v27.cuf        # Stage 2: CUDA Fortran training
├── compile_cifar10_v27.sh       # Compilation script
├── cifar10_data/                # Generated by Python
│   ├── images_train.bin
│   ├── labels_train.bin
│   ├── images_test.bin
│   └── labels_test.bin
└── cifar-10-batches-py/         # Downloaded by Python script
...
### Use v27 if:
- ✅ You're new to CUDA Fortran
- ✅ You want familiar Python data handling
- ✅ You need to modify preprocessing easily
- ✅ You want simpler, more maintainable code
- ✅ You're following the Oxford Flowers pattern

### Use v26 if:
- ✅ You want pure Fortran (no Python dependency)
- ✅ You're debugging low-level data issues
- ✅ You need maximum control over every step
- ✅ You're a Fortran purist
...
**The best of both worlds**: Python's ease + Fortran's speed = Perfect ML workflow 🚀

...
class TensorMatrixOps:
    """GPU-accelerated matrix operations using CUDA Fortran tensor cores (v5).
    ...
    matmul_vector(a, v)                  A @ v  (DGEMV)
    batched_vector_matmul(v, a)          Batched A @ v (TF32)
    tensor_4d_matmul(a, b)              4-D batched matmul
    tensor_5d_matmul(a, b)              5-D batched matmul

    v5: cuBLAS-lt epilogue fusion (new in v5)
    -----------------------------------------
    batched_matmul_bias_relu(a, b, bias)
        Computes relu(a @ b + bias) in a single fused GEMM kernel.
        Bias and ReLU run in the GEMM output registers before the result
        is written to global memory, eliminating one full memory round-trip
        versus the unfused pattern:
            c = batched_matmul(a, b)
            c = cp.maximum(c + bias[:, None], 0.0)

        Measured speedups on RTX 4060 (min over 10 runs):

        F/U = fused vs unfused TF32 (fusion benefit only):
            (8, 128³)           1.12×   ← crossover point
            (8, 256³)           1.29×
            (8, 1024³)          1.16×
            fc1 (512×2048×128)  1.24×
            fc2 (256×512×128)   1.57×
            fc3 (10×256×128)    1.67×

        F/C = fused vs CuPy FP64 (full replacement gain, TF32 + fusion):
            (8, 256³)           7.77×
            (8, 1024³)         20.13×
            (8, 2048³)         30.46×
            fc1 (512×2048×128) 12.03×
            fc2 (256×512×128)   3.35×

        F/C is dominated by TF32 vs FP64 (~10–20×); fusion adds 4–67%
        on top. Do not read F/C as the fusion-only benefit.

        CROSSOVER WARNING: for output matrices smaller than ~16,000
        elements per batch slice (roughly 128³ per element), the
        cuBLAS-lt fixed overhead exceeds the round-trip savings and
        fused is ~10% SLOWER than unfused:
            (8, 16³)   F/U = 0.88×  — use batched_matmul instead
            (8, 32³)   F/U = 0.89×  — use batched_matmul instead
            (8, 64³)   F/U = 0.90×  — use batched_matmul instead

        When to use: output size per batch element ≥ ~16K elements
        (all FC layer shapes clear this threshold comfortably).

    batched_matmul_bias(a, b, bias)
        Same as batched_matmul_bias_relu but without ReLU. Use for the
        last FC layer (pre-softmax) or any linear/separate activation.
        Same crossover threshold applies.

    Function selection guide
    ------------------------
    GEMM + bias + relu, output ≥ ~16K elements  →  batched_matmul_bias_relu
    GEMM + bias only,   output ≥ ~16K elements  →  batched_matmul_bias
    GEMM only, OR output < ~16K elements         →  batched_matmul
    FP64 accuracy required                       →  batched_matmul_fp64
    Replacing cp.matmul + bias + relu anywhere   →  batched_matmul_bias_relu (3–30× gain)

    Implementation notes
    --------------------
    Both fused functions are backed by cublaslt_bridge.c which holds a
    persistent cublasLtHandle_t and a 16-entry algo cache keyed by
    (M, K, N, epilogue). The cuBLAS-lt heuristic runs once per unique
    shape; subsequent calls reuse the cached algorithm with zero overhead.
    Workspace: 4 MB static GPU allocation shared across all calls.
    """


    # Then modify your __init__ method to use the function:
    def __init__(self, lib_path=None, tensor_cores=None):
        import os
        if lib_path is None:
            lib_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'cuda_matlib.so')
    ...
    def _setup_functions(self):
        ...
        self.lib.py_batched_matmul_fp64.argtypes = [
            ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p,
            ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int
        ]
        ...
        self.lib.py_batched_matmul_bias_relu.argtypes = [
            ctypes.c_void_p,  # a  (batch, m, k) fp64
            ctypes.c_void_p,  # b  (batch, k, n) fp64
            ctypes.c_void_p,  # c  (batch, m, n) fp64
            ctypes.c_void_p,  # bias (m,) fp64
            ctypes.c_int,     # m
            ctypes.c_int,     # k
            ctypes.c_int,     # n
            ctypes.c_int,     # batch_size
        ]
        self.lib.py_batched_matmul_bias_relu.restype = None

        self.lib.py_batched_matmul_bias.argtypes = [
            ctypes.c_void_p,  # a
            ctypes.c_void_p,  # b
            ctypes.c_void_p,  # c
        ...
        self.lib.py_batched_matmul_bias.restype = None
        ...
        self.lib.py_strided_batch_multiply.argtypes = [
            ...
            ctypes.c_int,                                        # batch_size
            ctypes.c_int64, ctypes.c_int64, ctypes.c_int64,      # a (batch,row,col)
            ctypes.c_int64, ctypes.c_int64, ctypes.c_int64,      # b (batch,row,col)
            ctypes.c_int64, ctypes.c_int64, ctypes.c_int64,      # c (batch,row,col)
        ]
        ...
        self.lib.py_tensor_5d_matmul.restype = None

        # v4: workspace pool cleanup
        self.lib.py_workspace_cleanup.argtypes = []
        self.lib.py_workspace_cleanup.restype = None

        print("Function signatures configured")

    def workspace_cleanup(self):
        """Release the persistent FP32/FP16 device workspace pools.

        The pool grows on demand and is reused across calls (zero-cost when the
        same shape is reused, as in a training loop).  Call this at session end
        or when you want to reclaim the device memory explicitly.
        """
        self.lib.py_workspace_cleanup()
    ...
    def batched_vector_matmul(self, v, a):
        """Batched matrix-vector products with a shared matrix: A·v_b per batch.

        NOTE (v5.1 doc fix): despite the name, this computes A·v for each
        batch column (the batched analog of matmul_vector, NOT vector_matmul).
        Layout handling is correct (a_padded is built Fortran-order); only the
        original docstring was misleading.  Behavior is unchanged.
    ...
    def batched_matmul_bias_relu(self, a, b, bias):
        ...
        Args:
            a:    (batch, m, k) fp64
            b:    (batch, k, n) fp64
            bias: (m,) fp64 — one value per output row
        ...
        a_gpu    = cp.ascontiguousarray(cp.asarray(a.reshape(batch_size, m, k), dtype=cp.float64))
        b_gpu    = cp.ascontiguousarray(cp.asarray(b.reshape(batch_size, k, n), dtype=cp.float64))
        c_gpu    = cp.empty((batch_size, m, n), dtype=cp.float64)
        bias_gpu = cp.ascontiguousarray(cp.asarray(bias, dtype=cp.float64))

        self.lib.py_batched_matmul_bias_relu(
            ctypes.c_void_p(a_gpu.data.ptr),
            ctypes.c_void_p(b_gpu.data.ptr),
            ctypes.c_void_p(c_gpu.data.ptr),
            ctypes.c_void_p(bias_gpu.data.ptr),
    ...
    def batched_matmul_bias(self, a, b, bias):
        ...
        self.lib.py_batched_matmul_bias(
            ctypes.c_void_p(a_gpu.data.ptr),
            ctypes.c_void_p(b_gpu.data.ptr),
            ctypes.c_void_p(c_gpu.data.ptr),
            ctypes.c_void_p(bias_gpu.data.ptr),
    ...
    def batched_matmul_fp64(self, a, b):
        """Compute batched matrix multiplication with FP64 precision.

        Uses cublasDgemmStridedBatched for perfect accuracy (e-13 to e-16) but slower
        performance (~40x slower than tensor core version). Use when accuracy is critical.
        ...
        # CRITICAL: Ensure C-order for strided batched GEMM
        # Convert to CuPy if needed, then ensure C-contiguous
        ...
        self.lib.py_batched_matmul_fp64(
            ctypes.c_void_p(a_gpu.data.ptr),
            ctypes.c_void_p(b_gpu.data.ptr),
            ctypes.c_void_p(c_gpu.data.ptr),
            m, k, n, batch_size
    ...
    def vector_matmul(self, v, a):
        ...
        # v5.1 FIX: must be Fortran-order — the Fortran kernel assumes
        # column-major A.  With the old cp.asarray (C-order kept) Fortran saw
        # A^T and this function returned A.v instead of the documented v.A.
        a_gpu = cp.asfortranarray(cp.asarray(a, dtype=cp.float64))
    ...
    def matmul_vector(self, a, v):
        ...
        Returns:
            Result vector (n,)
        """
        # v5.1 FIX: Fortran-order required (see vector_matmul) — the old
        # C-order pass-through made this function return v.A instead of A.v.
        a_gpu = cp.asfortranarray(cp.asarray(a, dtype=cp.float64))
        v_gpu = cp.asarray(v, dtype=cp.float64)
    ...
    def strided_batch_matmul(self, m, k, n, batch_size, a, b):
        """Compute batched C[i] = A[i] @ B[i] via the strided-batch kernel.

        v5.1 FIX: the old wrapper passed raw pointers (so the caller had to
        supply contiguous column-major slabs — which no caller did) and then
        reinterpreted the column-major Fortran output as row-major
        (batch, m, n), scrambling the result.  It now accepts standard
        (batch, m, k) / (batch, k, n) arrays in any memory order and returns
        (batch, m, n) matching cp.matmul(a, b) and the batched_matmul
        convention.

        Args:
            m, k, n:     per-batch GEMM dimensions
            batch_size:  number of matrices in the batch
            a:           (batch_size, m, k) FP64 NumPy or CuPy array
            b:           (batch_size, k, n) FP64 NumPy or CuPy array

        Returns:
            (batch_size, m, n) result; NumPy if `a` was NumPy, else CuPy.
        """
        m = int(m)
        k = int(k)
        n = int(n)
        ...
        if a_gpu.shape != (batch_size, m, k):
            raise ValueError(
                f"a must be (batch,m,k)=({batch_size},{m},{k}), got {a_gpu.shape}")
        if b_gpu.shape != (batch_size, k, n):
            raise ValueError(
                f"b must be (batch,k,n)=({batch_size},{k},{n}), got {b_gpu.shape}")

        # v5.2: hand the kernel the arrays' real element strides and let it read
        # through them. No host-side relayout in ANY memory order -- the v5.1
        # wrapper's three FP64 relayouts were 35-47% of the call, and merely
        # flipping the contract to C-order would just have moved that cost onto
        # the F-order arrays this benchmark actually passes.
        a_f, b_f = a_gpu, b_gpu
        c_f = cp.empty((batch_size, m, n), dtype=cp.float64)

        def _strides(arr):
            """(batch, row, col) strides in elements, not bytes."""
            it = arr.dtype.itemsize
            sb, sr, sc = arr.strides
            return ctypes.c_int64(sb // it), ctypes.c_int64(sr // it), ctypes.c_int64(sc // it)

        sab, sar, sac = _strides(a_f)
        sbb, sbr, sbc = _strides(b_f)
        scb, scr, scc = _strides(c_f)
        ...
        # Already (batch, m, n) C-order — no output relayout needed.
        return cp.asnumpy(c_f) if isinstance(a, np.ndarray) else c_f
